    ...
    public class CommonRoutines {
        ...
        public static string formatNameLFM(string lastName, string firstName, string mi) {
            string jdString=
                ...
                (isNothing(ObjectToStringV2(lastName))?"":", ") +
        ...
        public static string SocialSecurityNumberAtize(string ssn) {
            string jdssn = PadString(ssn, PAD_DIRECTION.LEFT, 9, '0');
            try {
        ...
        public static DataSet getDataSetSProc(SqlCommand cmd, string connectionString) {
        ...
        public static void deriveLatLongFromLocation(string address,out decimal? latitude,out decimal? longitude) {
            latitude = null;
            longitude = null;
            ...
            try {
                ...
                foreach (XmlNode objXmlNode in objXmlNodeList) {
                    // GET LONGITUDE 
                    latitude = Convert.ToDecimal(objXmlNode.ChildNodes.Item(0).InnerText);

                    // GET LATITUDE 
                    longitude= Convert.ToDecimal( objXmlNode.ChildNodes.Item(1).InnerText );
                }
            } catch {
        ...
        public static decimal ObjectToDecimal(object obj) {
            try {
                if (obj is DBNull || obj==null) {
                    return CommonRoutines.NULL_DECIMAL;
                } else {
        ...
        public static DateTime manageDateControl(string textInput) {
            DateTime dtDate = CommonRoutines.NULL_DATETIME;
            try {
        ...
        public static String timeInStringFormatFromNbrOfSecondsPastMidnight(int seconds) {
            if (seconds < 0) {
                return "";
            } else {
                DateTime today = DateTime.Today;
                today=today.AddSeconds(seconds);
                return today.ToShortTimeString();
            }
        }
